<?php
  ...
  if(isset($_POST['title']))//&&isset($_POST['description'])&&isset($_POST['url'])&&isset($_FILE['image']))
  {
    ...
    move_uploaded_file($_FILES['image']['tmp_name'],"uploads/1.png");
    $query = $pdo->prepare(
      "INSERT INTO `blog`
      (`uid`, `title`, `description`, `author`, `publish_date`, `url`, `image`)
      VALUES
      (:title,:description,:author,:pubish_date,:url,:image)");
